    ...
    for pattern, replacement, description in patterns:
        ...
        if new_content != content:
            print(
                ...
                f"\n{C.S} Applying Pattern {C.E} {C.G}➸❥ {C.P}{pattern}\n"
...
def Patch_Manifest(decompile_dir, manifest_path):
    ...
    print(f'\n{C.S} Updated {C.E} {C.C} android:networkSecurityConfig={C.P}"{C.G}@xml/network_security_config{C.P}" {C.G}➸❥ {C.Y}{M.os.path.basename(manifest_path)} {C.G}✔\n')

    print(f'\n{C.S} Updated {C.E} {C.C} android:usesCleartextTraffic={C.P}"{C.G}true{C.P}" {C.G}➸❥ {C.Y}{M.os.path.basename(manifest_path)} {C.G}✔\n')
...
def Permission_Manifest(decompile_dir, manifest_path, isAPKEditor):
    ...
    print(f"\n{C.S} Storage Permission {C.E} {C.G}➸❥ {C.P}'{C.G}AndroidManifest.xml{C.P}' {C.G}✔\n")
    ...
    if not isAPKEditor:
        ...
        print(f"\n{C.S} targetSdkVersion {C.E} {C.PN}28 {C.G}➸❥ apktool.yml\n")