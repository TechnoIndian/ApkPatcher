        ...
        if Regex.search(Smali):
            ...
            if Lock:
                try:
                    with Lock:
                        Count.value += 1
                        print(f"\r{C.S} Find Target Smali {C.E} {C.G}➸❥ {C.PN}{Count.value}", end='', flush=True)
                ...
                Count[0] += 1
                print(f"\r{C.S} Find Target Smali {C.E} {C.G}➸❥ {C.PN}{Count[0]}", end='', flush=True)
...
# ---------------- Apply Smali_Patch ----------------
def Smali_Patch(decompile_dir, smali_folders, isAPKEditor, CA_Cert, isID, isPairip, isPairip_lib, isPKG, isPurchased, isSC, Skip_Patch, isUSB, isCoreX):
    ...
    if matching_files:
        for pattern, replacement, description in patterns:
            ...
            if description in Skip_Patch:
                print(f"\n{C.S} Skip Patch {C.E} {C.G}➸❥ {description}\n")
                continue
            ...
            if count_applied > 0:
                ...
                print(f"\n{C.S} Pattern {C.E} {C.G}➸❥ {C.P}{pattern}")

                for file_path in applied_files:
                    print(f"{C.G}  |\n  └──── {C.CC}~{C.G}$ {C.Y}{M.os.path.basename(file_path)} {C.G}✔")

                print(
                    f"\n{C.S} Pattern Applied {C.E} {C.G}➸❥ {C.P}{count_applied} {C.C}Time/Smali {C.G}✔\n"
                    f"\n{C_Line}\n"
                )